...
# SETTINGS MENU
settings_menu_items = [
    # Header, then items under that heading
    ("MODAL CONFIG", "heading"),
    ("Mode", "mode"),  # selectable
    ("Keying Mode", "keying_mode"),  # selectable
    ...
    ("OTHER", "heading"),
    ("About", "about"),  # could trigger a separate display
    ("Reset", "reset"),  # could trigger a reset
]
...
def settings_menu_draw_menu():
    ...
    if settings_menu_current_menu_mode == settings_menu_in_editing_menu:
        ...
        label = item[0]
        key = item[1]
        if key != "heading":  # Only allow editing selectable items
    ...
    if settings_menu_selected == 1:  # "Mode" index
        extra_heading_line = 1  # scroll extra to show the "Modal Config" heading
    ...
    # Draw menu lines
    for i in range(settings_menu_scroll,
                   min(settings_menu_scroll + settings_menu_max_visible_lines, len(settings_menu_items))):
        ...
        if key == "heading":
            # Draw heading text
            oled.text(label, 4, y + 1, 1)
            # Draw underline using oled_gfx
            oled_gfx.fill_rect(4, y + 9, len(label) * 8, 1, 1)  # x, y, width, height, color
        else:
            # Regular menu items
            if i == settings_menu_selected:
                oled_gfx.fill_rect(0, y + 1, 128, settings_menu_line_height - 2, 1)
                oled.text(label, 12, y + 1, 0)
            else:
                oled.text(label, 12, y + 1, 1)
    oled.show()
...
def rotary_encoder_handler(delta):
    ...
    if settings_menu_current_menu_mode == settings_menu_in_scrollable_menu:
        ...
        while True:
            ...
            if settings_menu_items[next_index][1] != "heading":
                break
        ...
        if key == "wpm":
            settings[key] = min(max(10, menu_value + delta), 40)
        elif key == "volume":
            settings[key] = min(max(0, menu_value + delta), 10)
        elif key in ["led", "decoding"]:
            if delta != 0:
                settings[key] = not menu_value
        elif key == "mode":
