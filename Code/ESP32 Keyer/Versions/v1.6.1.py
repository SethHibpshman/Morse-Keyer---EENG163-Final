...
def draw_morse_oled(morse_oled_content=None, mode=None):
    ...
    if morse_oled_content is not None:
        ...
        oled.text(f'WPM: {int(wpm_static_max)}', 0, 24)
    if mode is not None:
        oled.text('Mode:', 0 , 36)
...
decoded_text = ""
def morse_decoder():
    '''When space detected (function called), crawl string. output all symbols between the space detected and the last space or slash.'''
    global decoded_text
    ...
    if morse_dictionary.get(last_morse_letter):
        ...
        print(morse_dictionary.get(last_morse_letter))
    else:
        decoded_text = decoded_text + '∅'
        print('∅')

def gap_checker():
    global morse_oled_content, idle_start_time, letter_space_happened, word_space_happened
    if dit_val == False and dah_val == False:
        ...
        if idle_time >= int(letter_break_t * 1000) and letter_space_happened == False:
            # CALL THE FUNCTION TO INTERPRET THE LETTER
            morse_decoder()
        ...
        if letter_space_happened == True and word_space_happened == False:
            if idle_time >= int((word_break_t) * 1000):
                ...
                if morse_oled_content and morse_oled_content[-1] == ' ':
                    ...
                    # ADD A SPACE IN ALPHANUMERIC PRINTOUT
                draw_morse_oled(morse_oled_content, keying_mode)
