...
while True:
    ...
    HOLD_THRESHOLD = 2000  # milliseconds

    if sw.value() == 0:  # Button pressed
        if press_start == 0:
            press_start = current_time
            press_handled = False
        elif not press_handled and ticks_diff(current_time, press_start) >= HOLD_THRESHOLD:
            # Long hold triggers menu toggle
            settings_menu_open = not settings_menu_open  # open if closed, close if open
            menu_mode = MODE_MENU if settings_menu_open else MODE_MENU  # reset menu mode when opening
