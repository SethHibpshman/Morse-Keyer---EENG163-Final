...
# SETTINGS MENU
settings_menu_items = ["WPM", "Mode", "Volume", "LED", "Keying Mode", "Decoding"]
# Values corisponding to each menu item. These are default values.
settings_menu_values = [
    10,    # WPM
    0,     # Mode index (Sandbox)
    5,     # Volume
    True,  # LED
    0,     # Keying Mode index (Iambic A)
    True,  # Decoding
]
...
def do_a_dit():
    ...
    led.value(settings_menu_values[3]) # Based on if LED is enabled.
...
def do_a_dah():
    global morse_oled_content, last_morse_end_time
    buz.duty_u16(buz_volume)
    led.value(settings_menu_values[3])
...
def settings_menu_draw_menu():
    ...
    for i in range(settings_menu_scroll, min(settings_menu_scroll + settings_menu_max_visible_lines, len(settings_menu_items))):
        ...
        if i == settings_menu_selected:
            oled_gfx.fill_rect(0, y, 128, settings_menu_line_height, 1) # Highlight bar
            oled.text(currently_selected_menu_item, 4, y + 1, 0) # Black text on highlight bar
        # Else draw normal.
        else:
...
def rotary_encoder_handler(delta):
    ...
    # If in the editing mode of the settings, menu, cycle through the settings values.
        ...
        elif settings_menu_selected == 1:  # Mode
            settings_menu_values[1] = (menu_value + delta) % len(modes)

        elif settings_menu_selected == 4:  # Keying Mode
            settings_menu_values[4] = (menu_value + delta) % len(keying_modes)
...
while True:
    ...
    # Dynamic Settings #################
    # Volume
    buz_volume = buz_vol_list[settings_menu_values[2]]  # The number in index 2 of the settings menu values (volume 0-10), corisponds to the pwm in the volume list.
    # Timing
    dit_t = 1.2 / settings_menu_values[0] # dit_t matched WPM calculations
    ...
    # Keying Mode
    keying_mode = keying_modes[settings_menu_values[4]]
