...
# SETTINGS MENU
settings_menu_items = [ # What to display, What to control
    ("WPM", "wpm"),
    ("Mode", "mode"),
    ("Volume", "volume"),
    ("LED", "led"),
...
def do_a_dit():
    ...
    led.value(settings["led"])  # Based on if LED is enabled.
...
def settings_menu_draw_menu():
    ...
    if settings_menu_current_menu_mode == settings_menu_in_editing_menu:
        item = settings_menu_items[settings_menu_selected]  # Get the tuple at selected index
        label = item[0]
        key = item[1]
        currently_selected_menu_item = label
        value_of_currently_selected_menu_item = value_to_str(settings_menu_selected, settings[key])
    ...
    for i in range(settings_menu_scroll, min(settings_menu_scroll + settings_menu_max_visible_lines, len(settings_menu_items))):
        y = (i - settings_menu_scroll) * settings_menu_line_height
        if y + settings_menu_line_height > 64:
            continue
        # Each menu item is (label, key)
        item = settings_menu_items[i]  # Get the tuple at index i
        label = item[0]    # First element is the display text
        key = item[1]      # Second element is the internal key
        currently_selected_menu_item = label  # Text to display on screen
...
def rotary_encoder_handler(delta):
    ...
    # If in the editing mode of the settings menu, cycle through the setting values
    elif settings_menu_current_menu_mode == settings_menu_in_editing_menu:
        # Get the currently selected menu item
        item = settings_menu_items[settings_menu_selected]  # tuple: (label, key)
        label = item[0]                                     # display text
        key = item[1]                                       # internal key

        # Get current value
        menu_value = settings[key]

        # Edit values based on the type of setting
        if key == "wpm":  # WPM setting
            settings[key] = min(max(10, menu_value + delta), 40)  # Clamp between 10-40

        elif key == "volume":  # Volume setting
            settings[key] = min(max(0, menu_value + delta), 10)   # Clamp between 0-10

        elif key in ["led", "decoding"]:  # Boolean switches
            if delta != 0:
                settings[key] = not menu_value  # Toggle True/False

        elif key == "mode":  # Cycling through modes
            settings[key] = (menu_value + delta) % len(modes)

        elif key == "keying_mode":  # Cycling through keying modes
